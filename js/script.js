...
function filterFruits() {
    ...
    fruitsFiltered = fruits.filter((item) => item.weight >= minValue && item.weight <= maxValue);
...
// сравнение массивов
const compareArrays = (a, b) => a.length === b.length && a.every((item, index) => item === b[index]);
...
document.addEventListener('click', (e) => {
    if (e.target.closest('.popup__close-btn') || e.target.closest('.popup__btn')) popup.classList.remove('popup-open');
...
// закрываем pop-up 2
document.addEventListener('click', (e) => {
    if (e.target.closest('.popup-add-fruit__btn') || e.target.closest('.popup-add-fruit__close-btn')) {
